...
echo -e "\n awk '{print}' test.log : Print all contents of the file"
awk '{print}' test.log

echo -e "\n awk '{print \$1}' test.log : Print only the first column"
awk '{print $1}' test.log

echo -e "\n awk '{print \$1, \$2}' test.log : Print first and second columns"
awk '{print $1, $2}' test.log

echo -e "\n awk '{print \$1, \$2, \$4}' test.log : Print first, second, and fourth columns"
awk '{print $1, $2, $4}' test.log

echo -e "\n awk '{print \$1, \$2, \$3, \$5}' test.log : Print selected columns"
awk '{print $1, $2, $3, $5}' test.log

echo -e "\n awk '/INFO/ {print \$1, \$2, \$3, \$5}' test.log : Print selected columns only for lines containing INFO"
awk '/INFO/ {print $1, $2, $3, $5}' test.log

echo -e "\n awk '/INFO/ {print \$1, \$2, \$3, \$5}' test.log > only-info.log : Save only INFO lines to a file"
awk '/INFO/ {print $1, $2, $3, $5}' test.log > only-info.log

echo -e "\n awk '/INFO/ {count++} END {print count}' only-info.log : Count number of INFO lines"
awk '/INFO/ {count++} END {print count}' only-info.log

echo -e "\n awk '\$2 >= \"08:53:00\" && \$2 <= \"08:54:00\" {print \$2, \$3, \$4}' test.log : Filter by time range"
...
echo -e "\n1 sed -n '/INFO/p' test.log : Print only lines with INFO"
sed -n '/INFO/p' test.log

echo -e "\n1 sed 's/INFO/LOG/g' test.log : Replace all 'INFO' with 'LOG'"
sed 's/INFO/LOG/g' test.log

echo -e "\n1 sed -n -e '/INFO/p' test.log : Same as above with explicit -e option"
sed -n -e '/INFO/p' test.log

echo -e "\n1 sed -n -e '/INFO/=' test.log : Print line numbers where INFO appears"
sed -n -e '/INFO/=' test.log

echo -e "\n1 sed -n -e '/INFO/=' -e '/INFO/p' test.log : Print line numbers and matching lines"
sed -n -e '/INFO/=' -e '/INFO/p' test.log


echo "========= GREP Commands ========="

echo -e "\n1 grep test.log : (Incorrect usage — should specify a pattern) Example shown below"
echo "✅ Correct Usage: grep INFO test.log"

echo -e "\n grep INFO test.log : Find all lines containing INFO"
grep INFO test.log

echo -e "\n grep -i info test.log : Case-insensitive search for 'info'"
grep -i info test.log

echo -e "\n grep -i -c info test.log : Count lines containing 'info' (case-insensitive)"
grep -i -c info test.log

echo -e "\n ps aux | grep ubuntu : Search for running processes related to 'ubuntu'"
ps aux | grep ubuntu
...
echo -e "\n sudo fdisk /dev/sda : Manage partitions (MBR - interactive)"
echo " Example: sudo fdisk /dev/sdb"

echo -e "\n sudo parted /dev/sdb : Manage partitions (supports GPT)"
sudo parted -l
...
echo -e "\n sudo blkid : Display UUIDs and filesystem info for block devices"
sudo blkid

echo -e "\n sudo mount /dev/sdb1 /mnt/data : Mount filesystem"
echo -e " sudo umount /mnt/data : Unmount filesystem"

echo -e "\n ls /dev/sd* : List all storage devices"
ls /dev/sd*
...
echo -e "\n1 sudo mkfs.ext4 /dev/sdb1 : Create ext4 filesystem on device"
echo -e "\n1 sudo fsck /dev/sda1 : Check and repair filesystem errors"
echo -e "\n1 sudo tune2fs -l /dev/sda1 : Display filesystem parameters"
echo -e "\n1 sudo resize2fs /dev/sda1 : Resize ext filesystem"
echo -e "\n1 sudo e2label /dev/sda1 DATA_DISK : Set/change filesystem label"


echo "========= LVM (LOGICAL VOLUME MANAGER) ========="

echo -e "\n sudo pvcreate /dev/sdb1 : Create Physical Volume (PV)"
echo -e "\n sudo vgcreate vg_data /dev/sdb1 : Create Volume Group (VG)"
echo -e "\n sudo lvcreate -n lv_backup -L 20G vg_data : Create Logical Volume (LV)"

echo -e "\n1 sudo pvdisplay : Display Physical Volume details"
sudo pvdisplay | head -n 10

echo -e "\n sudo vgdisplay : Display Volume Group details"
sudo vgdisplay | head -n 10

echo -e "\n sudo lvdisplay : Display Logical Volume details"
sudo lvdisplay | head -n 10

echo -e "\n sudo lvextend -L +10G /dev/vg_data/lv_backup : Extend LV by 10GB"
echo -e "\n sudo lvreduce -L 15G /dev/vg_data/lv_backup : Reduce LV size to 15GB (⚠️ risk of data loss)"

echo -e "\n sudo vgextend vg_data /dev/sdc1 : Add disk to Volume Group"
echo -e "\n sudo vgreduce vg_data /dev/sdc1 : Remove disk from Volume Group"

echo -e "\n sudo lvremove /dev/vg_data/lv_backup : Remove Logical Volume"
echo -e "\n sudo vgremove vg_data : Remove Volume Group"
echo -e "\n sudo pvremove /dev/sdb1 : Remove Physical Volume metadata"

echo -e "\n2 sudo pvs : Summary of all Physical Volumes"
sudo pvs

echo -e "\n sudo vgs : Summary of all Volume Groups"
sudo vgs

echo -e "\n sudo lvs : Summary of all Logical Volumes"
sudo lvs


echo "=========  MOUNT & PERSISTENT STORAGE ========="

echo -e "\n sudo mount -a : Mount all filesystems from /etc/fstab"
sudo mount -a

echo -e "\n cat /etc/fstab : View persistent mount configurations"
cat /etc/fstab | head -n 10

echo -e "\n lsblk -f : List filesystems with mount points"
lsblk -f

echo -e "\n ls -lh /dev/disk/by-uuid/ : Show disks by UUID"
ls -lh /dev/disk/by-uuid/


echo "=========  SYSTEM & STORAGE INFORMATION ========="

echo -e "\n3 df -Th : Show filesystem types with disk usage"
df -Th

echo -e "\n sudo parted -l : Check all partitions on all disks"
sudo parted -l | head -n 10

echo -e "\n du -h /home : Display disk usage of /home directory"
du -h /home | head -n 10

echo -e "\n mount | column -t : Show mounted filesystems neatly"
mount | column -t | head -n 10

echo -e "\n iostat : Display disk I/O statistics"
iostat | head -n 10

echo -e "\n cat /proc/partitions : Display kernel partition table"
cat /proc/partitions

echo -e "\n sudo hdparm -I /dev/sda : Show hardware info for disk"
